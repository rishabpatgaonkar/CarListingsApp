...
def submit():
    ...
    try:
        ...
        # color and price
            ...
            sql_select_Query = """select * from car where c_color = %s and c_price = %s"""
            cursor = conn.cursor()
            cursor.execute(sql_select_Query, [color, price])
        ...
        # remove unecessary characters from list
        newP = (','.join(str(a)for a in p))
        p = ''.join(str(newP).split(','))
